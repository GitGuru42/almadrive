...
def main_menu_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton("📝 Отзывы о сервисе"), KeyboardButton("⭐ Отзывы к авто")],
...
def _review_keyboard(review_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[
            InlineKeyboardButton("✅ Одобрить", callback_data=f"rev_appr:{review_id}"),
            InlineKeyboardButton("🗑️ Удалить", callback_data=f"rev_del:{review_id}"),
        ]]
    )
...
async def process_class_multiplier_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await api_request("PUT", f"/api/admin/vehicle-classes/{class_id}/multiplier", json={"price_multiplier": multiplier})
        await safe_reply(
            update,
            context,
...
@admin_only
async def reviews_pending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        items = await api_request("GET", "/api/reviews/pending", params={"limit": "20"})
    except Exception as e:
        await safe_reply(update, context, f"❌ Ошибка запроса: {e}")
        return

    if not items:
        await safe_reply(update, context, "✅ Нет отзывов на модерацию.", reply_markup=main_menu_keyboard())
        return

    await safe_reply(update, context, f"🛡️ На модерации: {len(items)}", reply_markup=main_menu_keyboard())
    ...
    for r in items:
        ...
        await safe_reply(update, context, msg, reply_markup=_review_keyboard(int(rid)))


async def review_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not query:
        return
    ...
    try:
        if action == "rev_appr":
            await api_request("PUT", f"/api/reviews/{rid}/approve")
            await safe_edit_or_send(query, context, f"✅ Отзыв #{rid} одобрен")
        elif action == "rev_del":
            await api_request("DELETE", f"/api/reviews/{rid}")
            await safe_edit_or_send(query, context, f"🗑️ Отзыв #{rid} удалён")
    except Exception as e:
        await safe_edit_or_send(query, context, f"❌ Ошибка: {e}")
...
async def menu_buttons_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "⭐ Отзывы к авто":
        return await reviews_pending(update, context)
...
def main():
    ...
    app.add_handler(CommandHandler("reviews_pending", reviews_pending))
    app.add_handler(CommandHandler("tariffs", tariffs_show))

    app.add_handler(MessageHandler(filters.Regex(r"^(🚗 Автомобили|💰 Тарифы|📝 Отзывы о сервисе|⭐ Отзывы к авто|➕ Добавить авто|❌ Отмена)$"), menu_buttons_handler))

    app.add_handler(CallbackQueryHandler(review_action_callback, pattern=r"^rev_(appr|del):"))
    ...
    app.add_handler(CallbackQueryHandler(tariffs_show_callback, pattern=r"^menu_cars$"))
    ...
    price_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(select_service_price_callback, pattern=r"^set_srv_price:"),
            CallbackQueryHandler(select_class_multiplier_callback, pattern=r"^set_cls_mult:"),
        ],
        states={
